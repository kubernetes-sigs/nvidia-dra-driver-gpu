...
func NewDriver(ctx context.Context, config *Config) (_ *driver, retErr error) {
	...
	helper, err := kubeletplugin.Start(ctx, driver, kubeletPluginOptions(driver.client, config)...)
	if err != nil {
		return nil, err
	}
...
func (d *driver) deviceHealthHeartbeats(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			...
			return
		case _, ok := <-d.deviceHealthMonitor.Heartbeat():
			if !ok {
				// The monitor is stopped; see nvmlDeviceHealthMonitor.Stop.
				klog.V(6).Info("Device health monitor heartbeat channel closed; stop processing device health heartbeats")
				return
			}
...
// kubeletPluginOptions returns the options NewDriver starts the kubeletplugin
// helper with. It is separate from NewDriver so that a test can start the
// helper with exactly these options and check what it advertises to the
// kubelet; the helper applies options in order, so a later option silently
// overrides an earlier one for the same setting.
func kubeletPluginOptions(client coreclientset.Interface, config *Config) []kubeletplugin.Option {
