...
func (m *nvmlDeviceHealthMonitor) Stop() {
	...
	// Closing both channels lets the driver's consumer goroutines exit
	// even when their context is not canceled, for example when NewDriver
	// fails after starting them and stops the monitor on the way out.
	close(m.unhealthy)
	if m.heartbeat != nil {
		close(m.heartbeat)
	}
...
// update. It reads the allocatable device map without the state lock; see
// the concurrency note on PerGPUAllocatableDevices for why that is safe.
...
//
// The fan-out to the MIG devices is required by device health reporting
// (KEP-4680): a pod is allocated a MIG device, not its parent GPU, and the
// health reported for it is derived from that device's own taints
// (deviceHealthFromTaints). Tainting only the full GPU, or dropping the event
// when the full GPU is not allocatable, would leave a pod on a MIG slice
// reporting Healthy on a GPU that fell off the bus, and the scheduler would
// keep placing new pods on its MIG devices.
//
// Reads the allocatable device map without the state lock; see the
// concurrency note on PerGPUAllocatableDevices for why that is safe.
