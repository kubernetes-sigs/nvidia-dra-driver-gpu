...
// tracked and the XID taint is cleared.) Keeping the taint is required by
// device health reporting (KEP-4680), which derives a device's health from
// its own taints: clearing the fanned-out taint here would report the next
// pod on this profile as Healthy while the GPU is still critical.
//
