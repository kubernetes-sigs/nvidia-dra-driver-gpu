...
// TestDeviceHealthConsumers_ExitWhenMonitorStops verifies that both consumer
// goroutines return when the monitor closes its channels, without their
// context being canceled: NewDriver relies on this when it fails after
// starting them and stops the monitor on the error path.
func TestDeviceHealthConsumers_ExitWhenMonitorStops(t *testing.T) {
	monitor := &mockHealthMonitor{
		heartbeatCh: make(chan struct{}, 1),
		unhealthyCh: make(chan *DeviceHealthEvent, 1),
	}
	d, _ := newHealthTestDriver(monitor)
	d.startDeviceHealthConsumers(context.Background())

	close(monitor.unhealthyCh)
	close(monitor.heartbeatCh)
	...
	case <-done:
	case <-time.After(5 * time.Second):
		t.Fatal("health consumers did not exit after the monitor closed its channels")
	}
}
...
// re-sent snapshot must never carry a fresh timestamp for stale health, and
// conversely a health change is a fresh determination, so it must not carry
// the timestamp of the previous heartbeat.
func TestDeviceHealth_TaintChangeReportedWhileStateLockBusy(t *testing.T) {
	...
	assert.False(t, report.Devices[0].LastUpdated.Before(before), "a health change must carry the time it was determined")
}
...
// TestKubeletPluginOptions_HealthServiceFollowsGate starts the kubeletplugin
// helper with exactly the options NewDriver passes it and checks, through the
// plugin registration API the kubelet uses, that the DRAResourceHealth
// service is advertised if and only if NVMLDeviceHealthCheck is enabled. The
// helper applies options in order, so this also catches a later option
// overriding the gated one, which the unit tests of the health code itself
// cannot see.
func TestKubeletPluginOptions_HealthServiceFollowsGate(t *testing.T) {
	for _, enabled := range []bool{true, false} {
		t.Run(fmt.Sprintf("NVMLDeviceHealthCheck=%t", enabled), func(t *testing.T) {
			require.NoError(t, featuregates.FeatureGates().SetFromMap(map[string]bool{
				string(featuregates.NVMLDeviceHealthCheck): enabled,
			}))
			t.Cleanup(func() {
				require.NoError(t, featuregates.FeatureGates().SetFromMap(map[string]bool{
					string(featuregates.NVMLDeviceHealthCheck): false,
			...
			// Not t.TempDir(): the socket paths below must stay under the
			// Unix socket path limit, and t.TempDir() embeds the test name.
			root, err := os.MkdirTemp("", "hs")
			require.NoError(t, err)
			t.Cleanup(func() { _ = os.RemoveAll(root) })
			config := &Config{flags: &Flags{
				nodeName:                      "node1",
				kubeletRegistrarDirectoryPath: filepath.Join(root, "plugins_registry"),
				kubeletPluginsDirectoryPath:   filepath.Join(root, "plugins"),
			}}
			require.NoError(t, os.MkdirAll(config.flags.kubeletRegistrarDirectoryPath, 0o755))
			require.NoError(t, os.MkdirAll(config.DriverPluginPath(), 0o755))

			d, _ := newHealthTestDriver(&mockHealthMonitor{})
			helper, err := kubeletplugin.Start(t.Context(), d, kubeletPluginOptions(k8sfake.NewSimpleClientset(), config)...)
			require.NoError(t, err)
			t.Cleanup(helper.Stop)

			// Ask the registrar what it would tell the kubelet.
			socks, err := filepath.Glob(filepath.Join(config.flags.kubeletRegistrarDirectoryPath, "*-reg.sock"))
			require.NoError(t, err)
			require.Len(t, socks, 1, "one registrar socket")
			conn, err := grpc.NewClient("unix:"+socks[0], grpc.WithTransportCredentials(insecure.NewCredentials()))
			require.NoError(t, err)
			t.Cleanup(func() { _ = conn.Close() })
			info, err := registerapi.NewRegistrationClient(conn).GetInfo(t.Context(), &registerapi.InfoRequest{})
			require.NoError(t, err)

			// DRA reports the gRPC services it provides in the registration
			// API's SupportedVersions field.
			assert.Equal(t, enabled, slices.Contains(info.SupportedVersions, drahealthv1.DRAResourceHealthService),
				"advertised services %v must include %s iff the gate is enabled", info.SupportedVersions, drahealthv1.DRAResourceHealthService)
		})
	}
}
