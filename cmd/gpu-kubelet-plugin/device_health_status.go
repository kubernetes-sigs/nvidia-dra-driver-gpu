...
// both.
//
// The VFIO cases in this file cannot run today: VFIO devices only exist
// under PassthroughSupport, which is mutually exclusive with
// NVMLDeviceHealthCheck, and without the latter the health service is not
// advertised at all. They are kept because they are the correct answer if
// that exclusivity is ever lifted, not because any current path reaches
// them.
...
func deviceHealthFromTaints(dev *AllocatableDevice) (kubeletplugin.HealthStatus, string) {
	...
	case dev.Type() == VfioDeviceType:
		// Defensive, unreachable today: see the note at the top of the file.
...
// re-discovered, though the last cannot happen today, see the note at the top
// of the file): it refreshes the health snapshot from the devices, which
// also stamps it with the current time since the health of every device was
// just re-determined, and wakes the WatchHealthStatus subscribers so they
// re-send it. The refresh waits for the state lock: the callers have just
// modified the devices under that lock themselves, so it is free or about to
// be, and the snapshot must reflect the change before the subscribers wake,
// whatever claim operation runs next.
...
// taints, waiting for the state lock if a claim operation holds it, and
// stamps it with the current time: the snapshot is the health as determined
// now, which is what LastUpdated reports. NewDriver calls it once before the
// kubelet can subscribe, so that the initial report always covers all
// devices; deviceHealthChanged calls it after every taint or device set
// change.
